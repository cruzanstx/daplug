---
name: devstral-cli
description: Run task with local Devstral model via Codex CLI + LMStudio (user)
argument-hint: <task> [--worktree /path]
---

Run a task using the local Devstral Small model via Codex CLI connected to LMStudio.
...
2. **After user responds, add config to `~/.codex/config.toml`:**
```bash
CONFIG=~/.codex/config.toml
...
- **Model**: Devstral Small 2 (mistralai/devstral-small-2-2512) - Mistral's compact coding model
- **Profile**: `local-devstral` (separate from `local` used by qwen)
- **LMStudio**: Must have Devstral Small loaded before running
