...
# Install daplug
claude plugin install daplug@daplug
```
...
claude plugin uninstall daplug@daplug
...
notify-plugin-update:
  ...
  script:
    - |
      ...
      #   --data "{\"text\":\"daplug plugin updated to v${VERSION} - run: claude plugin uninstall daplug@daplug && claude plugin marketplace update daplug && claude plugin install daplug@daplug\"}" \
      #   $SLACK_WEBHOOK_URL
