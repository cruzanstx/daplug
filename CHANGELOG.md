...
### Fixed
- **Prompt numbers are coordinated across linked worktrees** (#23) — global allocation scans every linked worktree and records the highest allocation in the shared Git directory. OS-backed locking prevents parallel creators from minting duplicate numbers, and explicit allocations remain reserved after their originating worktree disappears.

## [0.40.8] - 2026-09-03

### Changed
- **Gemini default shorthands (`gemini`, `agy`, `gemini37*`) now target Gemini 3.8 Flash** — bare `gemini`, `agy`, and the `gemini37*` shorthands now route to `google:gemini-3.8-flash` with Antigravity display names `Gemini 3.8 Flash (High)`, `Gemini 3.8 Flash (Medium)`, `Gemini 3.8 Flash (Low)`. The legacy Gemini CLI fallback becomes `gemini -y -m gemini-3.8-flash -p`.

## [0.40.7] - 2026-09-03

### Added
- **`fable51` model shorthand** — explicit, version-pinned shorthand for Claude Fable 5.1 via headless Claude Code CLI (`claude --print ... --model claude-fable-5-1`), following the same floating-alias-plus-explicit-pin convention as `glm5`/`glm52`/`glm53`. Canonical Claude API model ID `claude-fable-5-1`; released 2026-09-01 with a 1M context window, 128K max output tokens, adaptive thinking, effort levels low/medium/high/xhigh/max, and pricing of $10/MTok input, $50/MTok output (https://platform.claude.com/docs/en/models/fable-5-1/overview). The pre-existing `fable` shorthand is unchanged and continues to float to whatever Claude Code resolves the `fable` alias to — Claude Code v2.1.255+ resolves it to Fable 5.1 unless `ANTHROPIC_DEFAULT_FABLE_MODEL` overrides it (https://docs.anthropic.com/en/docs/claude-code/model-config).
