...
DEFAULT_OUTPUT_DIR = Path.home() / ".codex" / "prompts"
ARCHIVE_DIR_NAME = ".archive-pre-bridge"
BRIDGE_SENTINEL = "<!-- daplug-bridge: managed; do not edit -->"
...
def _render_bridge(command_name: str, command_spec: Path) -> str:
    ...
    return (
        ...
        "\n"
        "Read the canonical command specification at this absolute path,\n"
        "then execute it exactly as documented:\n"
        "\n"
        f"  {spec_path}\n"
        "\n"
...
def _is_managed_bridge(path: Path) -> bool:
    try:
        return BRIDGE_SENTINEL in path.read_text(encoding="utf-8")
    except (OSError, UnicodeDecodeError):
        return False


def _archive_handport(target: Path, archive_dir: Path) -> Path:
    archive_dir.mkdir(parents=True, exist_ok=True)
    dest = archive_dir / target.name
    counter = 1
    while dest.exists():
        dest = archive_dir / f"{target.stem}.{counter}{target.suffix}"
        counter += 1
    target.replace(dest)
    return dest
...
def generate_bridges(*, plugin_root: Path, output_dir: Path, clean: bool) -> int:
    ...
    if clean:
        ...
        print(f"Removed {removed} stale managed bridge(s) from {output_dir}")

    generated = 0
    archived = 0
    for spec in command_specs:
        command_name = spec.stem
        bridge_path = output_dir / f"{command_name}.md"
        if bridge_path.exists() and not _is_managed_bridge(bridge_path):
            dest = _archive_handport(bridge_path, archive_dir)
            print(f"Archived hand-port {bridge_path} -> {dest}")
            archived += 1
    ...
    print(
        ...
        f"(plugin_root={plugin_root}, archived={archived}, cleaned={removed})"
    )
    return generated
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Generate Codex-compatible daplug command bridge wrappers."
    )
    parser.add_argument(
        "output_dir",
        nargs="?",
    ...
    parser.add_argument(
        "--clean",
        action="store_true",
        help="Remove existing managed daplug bridges (identified by sentinel) before regenerating.",
    )
